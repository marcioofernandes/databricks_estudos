...
SELECT *
FROM text.`/Volumes/dbacademy/ops/labuser15060561_1778498302@vocareum_com/csv_demo_files/lab_malformed_data.csv`;
...
SELECT *
FROM read_files(
    '/Volumes/dbacademy/ops/labuser15060561_1778498302@vocareum_com/csv_demo_files/lab_malformed_data.csv',
    format => 'csv',
    header => true, 
    sep => ",",
    schema => '''
...
CREATE OR REPLACE TABLE 05_lab_bronze as 
SELECT
* ,
...
CREATE OR REPLACE TABLE 05_lab_challenge AS 
SELECT
  item_id,
  name,
  price,
  coalesce(price, replace(_rescued_data:price,'$','')) as price_fixed,  -- CLEAN the rescued data column and return it as a numeric value with the other prices
  _rescued_data,
  _metadata.file_modification_time AS file_modification_time,
  _metadata.file_name AS source_file, 
